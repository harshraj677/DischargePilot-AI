...
export function ReviewHistoryTable({ entries }: ReviewHistoryTableProps) {
  if (entries.length === 0) {
    return (
      <EmptyState
        icon={<ClipboardList className="h-6 w-6" />}
        title="No review history found"
        description="Try adjusting your filters, or check back once findings have been reviewed."
      />
    );
  }
          ...
          {entries.map((entry) => (
            <tr key={entry.id} className="text-slate-700">
              ...
              <td className="py-2.5 pr-4 text-slate-500">
                {entry.comments ? truncate(entry.comments, 40) : "—"}
              </td>
            </tr>
          ))}
        </tbody>
