...
import { Sparkles, Database, Repeat, ScanText, Timer } from "lucide-react";
import { GroqStatusBadge } from "@/components/ui/Badge";
import { system as systemApi } from "@/lib/api";
import type { GroqStatus } from "@/lib/types";
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-slate-900">Groq Status</h2>
        </div>
        {loading ? (
          <div className="h-5 w-20 animate-pulse rounded-full bg-slate-100" />
        ) : status ? (
          <GroqStatusBadge status={status.status} />
        ) : null}
      </div>
      ...
      {error ? (
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
          ...
          <div>
            <p className="flex items-center gap-1 text-xs font-semibold uppercase tracking-wide text-slate-500">
              <Timer className="h-3 w-3" /> Rate Limit
            </p>
            <p className="mt-1 text-sm font-medium text-slate-900">{status.rate_limit.hits}</p>
            <p className="text-xs text-slate-400">429 retries handled</p>
          </div>
        </div>
      ) : null}
    </div>
