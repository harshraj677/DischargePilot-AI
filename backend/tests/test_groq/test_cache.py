"""
Tests for GroqResponseCache.

SHA256(document_content)-keyed file cache: if a document set has already been
processed, the cached structured extraction is reused and Groq is never
called again for it.
"""
import hashlib
...
@pytest.fixture
def cache(tmp_path):
    return GroqResponseCache(cache_dir=tmp_path / "groq_responses")
...
class TestHashContent:
    def test_hash_is_sha256_hexdigest(self):
        ...
        assert GroqResponseCache.hash_content(content) == expected

    def test_same_content_same_hash(self):
        content = "Some clinical document text"
        assert GroqResponseCache.hash_content(content) == GroqResponseCache.hash_content(content)

    def test_different_content_different_hash(self):
        assert GroqResponseCache.hash_content("doc A") != GroqResponseCache.hash_content("doc B")
...
class TestGetSet:
    ...
    def test_set_then_get_returns_same_data(self, cache):
        content_hash = GroqResponseCache.hash_content("doc text")
...
class TestCacheDisabled:
    def test_get_returns_none_when_disabled(self, cache, monkeypatch):
        ...
        monkeypatch.setattr(settings, "GROQ_CACHE_ENABLED", False)

        assert cache.get(content_hash) is None

    def test_set_is_noop_when_disabled(self, cache, monkeypatch):
        monkeypatch.setattr(settings, "GROQ_CACHE_ENABLED", True)
        content_hash = GroqResponseCache.hash_content("doc text")
