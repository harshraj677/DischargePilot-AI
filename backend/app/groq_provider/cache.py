...
structured extraction is reused and Groq is never called again for it —
this is the single-consolidated-call strategy that avoids 429 quota
exhaustion from repeated per-extractor calls.
"""
...
CACHE_DIR = BASE_DIR / "cache" / "groq_responses"


class GroqResponseCache:
    """File-based cache for Groq extraction responses, keyed by content hash."""
...
_cache_instance: Optional[GroqResponseCache] = None


def get_groq_response_cache() -> GroqResponseCache:
    global _cache_instance
    if _cache_instance is None:
        _cache_instance = GroqResponseCache()
